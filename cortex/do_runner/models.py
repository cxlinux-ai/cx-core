...
class TaskTree:
    ...
    def _print_node(self, node: TaskNode, indent: str, is_last: bool):
        ...
        console.print(f"{indent}{prefix}{icon} [{color}][{node.task_type.value}][/{color}] {node.command[:50]}...")
        
        if node.reasoning:
            console.print(f"{indent}{'    ' if is_last else '│   '}[dim]Reason: {node.reasoning}[/dim]")
