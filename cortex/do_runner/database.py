...
class DoRunDatabase:
    ...
    def _ensure_directory(self):
        ...
        try:
            ...
            console.print(f"[yellow]Warning: Using alternate database path: {self.db_path}[/yellow]")
    ...
    def get_run(self, run_id: str) -> DoRun | None:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            cursor = conn.execute(
                "SELECT * FROM do_runs WHERE run_id = ?", (run_id,)
            )
            row = cursor.fetchone()
    ...
    def get_commands_list(self, run_id: str) -> list[str]:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT commands_list FROM do_runs WHERE run_id = ?", (run_id,)
            )
            row = cursor.fetchone()
