...
COMMAND_SHELL_ERRORS = [
    ...
    ErrorPattern(r"[Cc]ommand timed out after \d+ seconds", "command_timeout", "timeout",
                 "Command timed out - operation took too long", True, "retry_with_longer_timeout"),
    ErrorPattern(r"[Tt]imed out", "timeout", "timeout",
                 "Operation timed out", True, "retry_with_longer_timeout"),
    ErrorPattern(r"[Tt]imeout", "timeout", "timeout",
                 "Operation timed out", True, "retry_with_longer_timeout"),
    # Standard command errors
    ErrorPattern(r"command not found", "command_not_found", "command_shell", 
                 "Command not installed", True, "install_package"),
    ErrorPattern(r"No such file or directory", "not_found", "command_shell",
                 "File or directory not found", True, "create_path"),
    ErrorPattern(r"Permission denied", "permission_denied", "command_shell",
                 "Permission denied", True, "use_sudo"),
    ErrorPattern(r"Operation not permitted", "operation_not_permitted", "command_shell",
                 "Operation not permitted (may need root)", True, "use_sudo"),
    ErrorPattern(r"Not a directory", "not_a_directory", "command_shell",
                 "Expected directory but found file", False, "check_path"),
    ErrorPattern(r"Is a directory", "is_a_directory", "command_shell",
                 "Expected file but found directory", False, "check_path"),
    ErrorPattern(r"Invalid argument", "invalid_argument", "command_shell",
                 "Invalid argument passed", False, "check_args"),
    ErrorPattern(r"Too many arguments", "too_many_args", "command_shell",
                 "Too many arguments provided", False, "check_args"),
    ErrorPattern(r"[Mm]issing operand", "missing_operand", "command_shell",
                 "Required argument missing", False, "check_args"),
    ErrorPattern(r"[Aa]mbiguous redirect", "ambiguous_redirect", "command_shell",
                 "Shell redirect is ambiguous", False, "fix_redirect"),
    ErrorPattern(r"[Bb]ad substitution", "bad_substitution", "command_shell",
                 "Shell variable substitution error", False, "fix_syntax"),
    ErrorPattern(r"[Uu]nbound variable", "unbound_variable", "command_shell",
                 "Variable not set", True, "set_variable"),
    ErrorPattern(r"[Ss]yntax error near unexpected token", "syntax_error_token", "command_shell",
                 "Shell syntax error", False, "fix_syntax"),
    ErrorPattern(r"[Uu]nexpected EOF", "unexpected_eof", "command_shell",
                 "Unclosed quote or bracket", False, "fix_syntax"),
    ErrorPattern(r"[Cc]annot execute binary file", "cannot_execute_binary", "command_shell",
                 "Binary incompatible with system", False, "check_architecture"),
    ErrorPattern(r"[Ee]xec format error", "exec_format_error", "command_shell",
                 "Invalid executable format", False, "check_architecture"),
    ErrorPattern(r"[Ii]llegal option", "illegal_option", "command_shell",
                 "Unrecognized command option", False, "check_help"),
    ErrorPattern(r"[Ii]nvalid option", "invalid_option", "command_shell",
                 "Invalid command option", False, "check_help"),
    ErrorPattern(r"[Rr]ead-only file ?system", "readonly_fs", "command_shell",
                 "Filesystem is read-only", True, "remount_rw"),
    ErrorPattern(r"[Ii]nput/output error", "io_error", "command_shell",
                 "I/O error (disk issue)", False, "check_disk", "critical"),
    ErrorPattern(r"[Tt]ext file busy", "text_file_busy", "command_shell",
                 "File is being executed", True, "wait_retry"),
    ErrorPattern(r"[Aa]rgument list too long", "arg_list_too_long", "command_shell",
                 "Too many arguments for command", True, "use_xargs"),
    ErrorPattern(r"[Bb]roken pipe", "broken_pipe", "command_shell",
                 "Pipe closed unexpectedly", False, "check_pipe"),
]

# Category 2: File & Directory Errors
FILE_DIRECTORY_ERRORS = [
    ErrorPattern(r"[Ff]ile exists", "file_exists", "file_directory",
                 "File already exists", True, "backup_overwrite"),
    ErrorPattern(r"[Ff]ile name too long", "filename_too_long", "file_directory",
                 "Filename exceeds limit", False, "shorten_name"),
    ErrorPattern(r"[Tt]oo many.*symbolic links", "symlink_loop", "file_directory",
                 "Symbolic link loop detected", True, "fix_symlink"),
    ErrorPattern(r"[Ss]tale file handle", "stale_handle", "file_directory",
                 "NFS file handle stale", True, "remount_nfs"),
    ErrorPattern(r"[Dd]irectory not empty", "dir_not_empty", "file_directory",
                 "Directory has contents", True, "rm_recursive"),
    ErrorPattern(r"[Cc]ross-device link", "cross_device_link", "file_directory",
                 "Cannot link across filesystems", True, "copy_instead"),
    ErrorPattern(r"[Tt]oo many open files", "too_many_files", "file_directory",
                 "File descriptor limit reached", True, "increase_ulimit"),
    ErrorPattern(r"[Qq]uota exceeded", "quota_exceeded", "file_directory",
                 "Disk quota exceeded", False, "check_quota"),
    ErrorPattern(r"[Oo]peration timed out", "operation_timeout", "file_directory",
                 "Operation timed out", True, "increase_timeout"),
]

# Category 3: Permission & Ownership Errors
PERMISSION_ERRORS = [
    ErrorPattern(r"[Aa]ccess denied", "access_denied", "permission",
                 "Access denied", True, "use_sudo"),
    ErrorPattern(r"[Aa]uthentication fail", "auth_failure", "permission",
                 "Authentication failed", False, "check_credentials"),
    ErrorPattern(r"[Ii]nvalid user", "invalid_user", "permission",
                 "User does not exist", True, "create_user"),
    ErrorPattern(r"[Ii]nvalid group", "invalid_group", "permission",
                 "Group does not exist", True, "create_group"),
    ErrorPattern(r"[Nn]ot owner", "not_owner", "permission",
                 "Not the owner of file", True, "use_sudo"),
]

# Category 4: Process & Execution Errors
PROCESS_ERRORS = [
    ErrorPattern(r"[Nn]o such process", "no_such_process", "process",
                 "Process does not exist", False, "check_pid"),
    ErrorPattern(r"[Pp]rocess already running", "already_running", "process",
                 "Process already running", True, "kill_existing"),
    ErrorPattern(r"[Pp]rocess terminated", "process_terminated", "process",
                 "Process was terminated", False, "check_logs"),
    ErrorPattern(r"[Kk]illed", "killed", "process",
                 "Process was killed (OOM?)", False, "check_memory", "critical"),
    ErrorPattern(r"[Ss]egmentation fault", "segfault", "process",
                 "Memory access violation", False, "debug_crash", "critical"),
    ErrorPattern(r"[Bb]us error", "bus_error", "process",
                 "Bus error (memory alignment)", False, "debug_crash", "critical"),
    ErrorPattern(r"[Ff]loating point exception", "fpe", "process",
                 "Floating point exception", False, "debug_crash"),
    ErrorPattern(r"[Ii]llegal instruction", "illegal_instruction", "process",
                 "CPU instruction error", False, "check_architecture", "critical"),
    ErrorPattern(r"[Tt]race.*trap", "trace_trap", "process",
                 "Debugger trap", False, "check_debugger"),
    ErrorPattern(r"[Rr]esource temporarily unavailable", "resource_unavailable", "process",
                 "Resource busy", True, "wait_retry"),
    ErrorPattern(r"[Tt]oo many processes", "too_many_processes", "process",
                 "Process limit reached", True, "increase_ulimit"),
    ErrorPattern(r"[Oo]peration canceled", "operation_canceled", "process",
                 "Operation was canceled", False, "check_timeout"),
]

# Category 5: Memory & Resource Errors
MEMORY_ERRORS = [
    ErrorPattern(r"[Oo]ut of memory", "oom", "memory",
                 "Out of memory", True, "free_memory", "critical"),
    ErrorPattern(r"[Cc]annot allocate memory", "cannot_allocate", "memory",
                 "Memory allocation failed", True, "free_memory", "critical"),
    ErrorPattern(r"[Mm]emory exhausted", "memory_exhausted", "memory",
                 "Memory exhausted", True, "free_memory", "critical"),
    ErrorPattern(r"[Ss]tack overflow", "stack_overflow", "memory",
                 "Stack overflow", False, "increase_stack", "critical"),
    ErrorPattern(r"[Dd]evice or resource busy", "device_busy", "memory",
                 "Device or resource busy", True, "wait_retry"),
    ErrorPattern(r"[Nn]o space left on device", "no_space", "memory",
                 "Disk full", True, "free_disk", "critical"),
    ErrorPattern(r"[Dd]isk quota exceeded", "disk_quota", "memory",
                 "Disk quota exceeded", False, "check_quota"),
    ErrorPattern(r"[Ff]ile table overflow", "file_table_overflow", "memory",
                 "System file table full", True, "increase_ulimit", "critical"),
]

# Category 6: Disk & Filesystem Errors
FILESYSTEM_ERRORS = [
    ErrorPattern(r"[Ww]rong fs type", "wrong_fs_type", "filesystem",
                 "Wrong filesystem type", False, "check_fstype"),
    ErrorPattern(r"[Ff]ilesystem.*corrupt", "fs_corrupt", "filesystem",
                 "Filesystem corrupted", False, "fsck", "critical"),
    ErrorPattern(r"[Ss]uperblock invalid", "superblock_invalid", "filesystem",
                 "Superblock invalid", False, "fsck", "critical"),
    ErrorPattern(r"[Mm]ount point does not exist", "mount_point_missing", "filesystem",
                 "Mount point missing", True, "create_mountpoint"),
    ErrorPattern(r"[Dd]evice is busy", "device_busy_mount", "filesystem",
                 "Device busy (in use)", True, "lazy_umount"),
    ErrorPattern(r"[Nn]ot mounted", "not_mounted", "filesystem",
                 "Filesystem not mounted", True, "mount_fs"),
    ErrorPattern(r"[Aa]lready mounted", "already_mounted", "filesystem",
                 "Already mounted", False, "check_mount"),
    ErrorPattern(r"[Bb]ad magic number", "bad_magic", "filesystem",
                 "Bad magic number in superblock", False, "fsck", "critical"),
    ErrorPattern(r"[Ss]tructure needs cleaning", "needs_cleaning", "filesystem",
                 "Filesystem needs fsck", False, "fsck"),
    ErrorPattern(r"[Jj]ournal has aborted", "journal_aborted", "filesystem",
                 "Journal aborted", False, "fsck", "critical"),
]

# Category 7: Networking Errors
NETWORK_ERRORS = [
    ErrorPattern(r"[Nn]etwork is unreachable", "network_unreachable", "network",
                 "Network unreachable", True, "check_network"),
    ErrorPattern(r"[Nn]o route to host", "no_route", "network",
                 "No route to host", True, "check_routing"),
    ErrorPattern(r"[Cc]onnection refused", "connection_refused", "network",
                 "Connection refused", True, "check_service"),
    ErrorPattern(r"[Cc]onnection timed out", "connection_timeout", "network",
                 "Connection timed out", True, "check_firewall"),
    ErrorPattern(r"[Cc]onnection reset by peer", "connection_reset", "network",
                 "Connection reset", False, "check_remote"),
    ErrorPattern(r"[Hh]ost is down", "host_down", "network",
                 "Remote host down", False, "check_host"),
    ErrorPattern(r"[Tt]emporary failure in name resolution", "dns_temp_fail", "network",
                 "DNS temporary failure", True, "retry_dns"),
    ErrorPattern(r"[Nn]ame or service not known", "dns_unknown", "network",
                 "DNS lookup failed", True, "check_dns"),
    ErrorPattern(r"[Dd]NS lookup failed", "dns_failed", "network",
                 "DNS lookup failed", True, "check_dns"),
    ErrorPattern(r"[Aa]ddress already in use", "address_in_use", "network",
                 "Port already in use", True, "find_port_user"),
    ErrorPattern(r"[Cc]annot assign requested address", "cannot_assign_addr", "network",
                 "Address not available", False, "check_interface"),
    ErrorPattern(r"[Pp]rotocol not supported", "protocol_not_supported", "network",
                 "Protocol not supported", False, "check_protocol"),
    ErrorPattern(r"[Ss]ocket operation on non-socket", "not_socket", "network",
                 "Invalid socket operation", False, "check_fd"),
]

# Category 8: Package Manager Errors (Ubuntu/Debian apt)
PACKAGE_ERRORS = [
    ErrorPattern(r"[Uu]nable to locate package", "package_not_found", "package",
                 "Package not found", True, "update_repos"),
    ErrorPattern(r"[Pp]ackage.*not found", "package_not_found", "package",
                 "Package not found", True, "update_repos"),
    ErrorPattern(r"[Ff]ailed to fetch", "fetch_failed", "package",
                 "Failed to download package", True, "change_mirror"),
    ErrorPattern(r"[Hh]ash [Ss]um mismatch", "hash_mismatch", "package",
                 "Package checksum mismatch", True, "clean_apt"),
    ErrorPattern(r"[Rr]epository.*not signed", "repo_not_signed", "package",
                 "Repository not signed", True, "add_key"),
    ErrorPattern(r"[Gg][Pp][Gg] error", "gpg_error", "package",
                 "GPG signature error", True, "fix_gpg"),
    ErrorPattern(r"[Dd]ependency problems", "dependency_problems", "package",
                 "Dependency issues", True, "fix_dependencies"),
    ErrorPattern(r"[Uu]nmet dependencies", "unmet_dependencies", "package",
                 "Unmet dependencies", True, "fix_dependencies"),
    ErrorPattern(r"[Bb]roken packages", "broken_packages", "package",
                 "Broken packages", True, "fix_broken"),
    ErrorPattern(r"[Vv]ery bad inconsistent state", "inconsistent_state", "package",
                 "Package in bad state", True, "force_reinstall"),
    ErrorPattern(r"[Cc]onflicts with", "package_conflict", "package",
                 "Package conflict", True, "resolve_conflict"),
    ErrorPattern(r"dpkg.*lock", "dpkg_lock", "package",
                 "Package manager locked", True, "clear_lock"),
    ErrorPattern(r"apt.*lock", "apt_lock", "package",
                 "APT locked", True, "clear_lock"),
    ErrorPattern(r"E: Could not get lock", "could_not_get_lock", "package",
                 "Package manager locked", True, "clear_lock"),
]

# Category 9: User & Authentication Errors
USER_AUTH_ERRORS = [
    ErrorPattern(r"[Uu]ser does not exist", "user_not_exist", "user_auth",
                 "User does not exist", True, "create_user"),
    ErrorPattern(r"[Gg]roup does not exist", "group_not_exist", "user_auth",
                 "Group does not exist", True, "create_group"),
    ErrorPattern(r"[Aa]ccount expired", "account_expired", "user_auth",
                 "Account expired", False, "renew_account"),
    ErrorPattern(r"[Pp]assword expired", "password_expired", "user_auth",
                 "Password expired", False, "change_password"),
    ErrorPattern(r"[Ii]ncorrect password", "wrong_password", "user_auth",
                 "Wrong password", False, "check_password"),
    ErrorPattern(r"[Aa]ccount locked", "account_locked", "user_auth",
                 "Account locked", False, "unlock_account"),
]

# Category 16: Docker/Container Errors
DOCKER_ERRORS = [
    # Container name conflicts
    ErrorPattern(r"[Cc]onflict.*container name.*already in use", "container_name_conflict", "docker",
                 "Container name already in use", True, "remove_or_rename_container"),
    ErrorPattern(r"is already in use by container", "container_name_conflict", "docker",
                 "Container name already in use", True, "remove_or_rename_container"),
    # Container not found
    ErrorPattern(r"[Nn]o such container", "container_not_found", "docker",
                 "Container does not exist", True, "check_container_name"),
    ErrorPattern(r"[Ee]rror: No such container", "container_not_found", "docker",
                 "Container does not exist", True, "check_container_name"),
    # Image not found
    ErrorPattern(r"[Uu]nable to find image", "image_not_found", "docker",
                 "Docker image not found locally", True, "pull_image"),
    ErrorPattern(r"[Rr]epository.*not found", "image_not_found", "docker",
                 "Docker image repository not found", True, "check_image_name"),
    ErrorPattern(r"manifest.*not found", "manifest_not_found", "docker",
                 "Image manifest not found", True, "check_image_tag"),
    # Container already running/stopped
    ErrorPattern(r"is already running", "container_already_running", "docker",
                 "Container is already running", True, "stop_or_use_existing"),
    ErrorPattern(r"is not running", "container_not_running", "docker",
                 "Container is not running", True, "start_container"),
    # Port conflicts
    ErrorPattern(r"[Pp]ort.*already allocated", "port_in_use", "docker",
                 "Port is already in use", True, "free_port_or_use_different"),
    ErrorPattern(r"[Bb]ind.*address already in use", "port_in_use", "docker",
                 "Port is already in use", True, "free_port_or_use_different"),
    # Volume errors
    ErrorPattern(r"[Vv]olume.*not found", "volume_not_found", "docker",
                 "Docker volume not found", True, "create_volume"),
    ErrorPattern(r"[Mm]ount.*denied", "mount_denied", "docker",
                 "Mount point access denied", True, "check_mount_permissions"),
    # Network errors
    ErrorPattern(r"[Nn]etwork.*not found", "network_not_found", "docker",
                 "Docker network not found", True, "create_network"),
    # Daemon errors
    ErrorPattern(r"[Cc]annot connect to the Docker daemon", "docker_daemon_not_running", "docker",
                 "Docker daemon is not running", True, "start_docker_daemon"),
    ErrorPattern(r"[Ii]s the docker daemon running", "docker_daemon_not_running", "docker",
                 "Docker daemon is not running", True, "start_docker_daemon"),
    # OOM errors
    ErrorPattern(r"[Oo]ut of memory", "container_oom", "docker",
                 "Container ran out of memory", True, "increase_memory_limit"),
    # Exec errors
    ErrorPattern(r"[Oo]CI runtime.*not found", "runtime_not_found", "docker",
                 "Container runtime not found", False, "check_docker_installation"),
]

# Category 17: Login/Credential Required Errors
LOGIN_REQUIRED_ERRORS = [
    # Docker/Container registry login errors
    ErrorPattern(r"[Uu]sername.*[Rr]equired", "docker_username_required", "login_required",
                 "Docker username required", True, "prompt_docker_login"),
    ErrorPattern(r"[Nn]on-null [Uu]sername", "docker_username_required", "login_required",
                 "Docker username required", True, "prompt_docker_login"),
    ErrorPattern(r"unauthorized.*authentication required", "docker_auth_required", "login_required",
                 "Docker authentication required", True, "prompt_docker_login"),
    ErrorPattern(r"denied.*requested access", "docker_access_denied", "login_required",
                 "Docker registry access denied", True, "prompt_docker_login"),
    ErrorPattern(r"denied:.*access", "docker_access_denied", "login_required",
                 "Docker registry access denied", True, "prompt_docker_login"),
    ErrorPattern(r"access.*denied", "docker_access_denied", "login_required",
                 "Docker registry access denied", True, "prompt_docker_login"),
    ErrorPattern(r"no basic auth credentials", "docker_no_credentials", "login_required",
                 "Docker credentials not found", True, "prompt_docker_login"),
    ErrorPattern(r"docker login", "docker_login_needed", "login_required",
                 "Docker login required", True, "prompt_docker_login"),
    # ghcr.io (GitHub Container Registry) specific errors
    ErrorPattern(r"ghcr\.io.*denied", "ghcr_access_denied", "login_required",
                 "GitHub Container Registry access denied - login required", True, "prompt_docker_login"),
    ErrorPattern(r"Head.*ghcr\.io.*denied", "ghcr_access_denied", "login_required",
                 "GitHub Container Registry access denied - login required", True, "prompt_docker_login"),
    # Generic registry denied patterns
    ErrorPattern(r"Error response from daemon.*denied", "registry_access_denied", "login_required",
                 "Container registry access denied - login may be required", True, "prompt_docker_login"),
    ErrorPattern(r"pull access denied", "pull_access_denied", "login_required",
                 "Pull access denied - login required or image doesn't exist", True, "prompt_docker_login"),
    ErrorPattern(r"requested resource.*denied", "resource_access_denied", "login_required",
                 "Resource access denied - authentication required", True, "prompt_docker_login"),
    
    # Git credential errors
    ErrorPattern(r"[Cc]ould not read.*[Uu]sername", "git_username_required", "login_required",
                 "Git username required", True, "prompt_git_login"),
    ErrorPattern(r"[Ff]atal:.*[Aa]uthentication failed", "git_auth_failed", "login_required",
                 "Git authentication failed", True, "prompt_git_login"),
    ErrorPattern(r"[Pp]assword.*authentication.*removed", "git_token_required", "login_required",
                 "Git token required (password auth disabled)", True, "prompt_git_token"),
    ErrorPattern(r"[Pp]ermission denied.*publickey", "git_ssh_required", "login_required",
                 "Git SSH key required", True, "setup_git_ssh"),
    
    # npm login errors
    ErrorPattern(r"npm ERR!.*E401", "npm_auth_required", "login_required",
                 "npm authentication required", True, "prompt_npm_login"),
    ErrorPattern(r"npm ERR!.*ENEEDAUTH", "npm_need_auth", "login_required",
                 "npm authentication needed", True, "prompt_npm_login"),
    ErrorPattern(r"You must be logged in", "npm_login_required", "login_required",
                 "npm login required", True, "prompt_npm_login"),
    
    # AWS credential errors
    ErrorPattern(r"[Uu]nable to locate credentials", "aws_no_credentials", "login_required",
                 "AWS credentials not configured", True, "prompt_aws_configure"),
    ErrorPattern(r"[Ii]nvalid[Aa]ccess[Kk]ey", "aws_invalid_key", "login_required",
                 "AWS access key invalid", True, "prompt_aws_configure"),
    ErrorPattern(r"[Ss]ignature.*[Dd]oes[Nn]ot[Mm]atch", "aws_secret_invalid", "login_required",
                 "AWS secret key invalid", True, "prompt_aws_configure"),
    ErrorPattern(r"[Ee]xpired[Tt]oken", "aws_token_expired", "login_required",
                 "AWS token expired", True, "prompt_aws_configure"),
    
    # PyPI/pip login errors
    ErrorPattern(r"HTTPError: 403.*upload", "pypi_auth_required", "login_required",
                 "PyPI authentication required", True, "prompt_pypi_login"),
    
    # Generic credential prompts
    ErrorPattern(r"[Ee]nter.*[Uu]sername", "username_prompt", "login_required",
                 "Username required", True, "prompt_credentials"),
    ErrorPattern(r"[Ee]nter.*[Pp]assword", "password_prompt", "login_required",
                 "Password required", True, "prompt_credentials"),
    ErrorPattern(r"[Aa]ccess [Tt]oken.*[Rr]equired", "token_required", "login_required",
                 "Access token required", True, "prompt_token"),
    ErrorPattern(r"[Aa][Pp][Ii].*[Kk]ey.*[Rr]equired", "api_key_required", "login_required",
                 "API key required", True, "prompt_api_key"),
]

# Category 10: Device & Hardware Errors
DEVICE_ERRORS = [
    ErrorPattern(r"[Nn]o such device", "no_device", "device",
                 "Device not found", False, "check_device"),
    ErrorPattern(r"[Dd]evice not configured", "device_not_configured", "device",
                 "Device not configured", False, "configure_device"),
    ErrorPattern(r"[Hh]ardware error", "hardware_error", "device",
                 "Hardware error", False, "check_hardware", "critical"),
    ErrorPattern(r"[Dd]evice offline", "device_offline", "device",
                 "Device offline", False, "bring_online"),
    ErrorPattern(r"[Mm]edia not present", "no_media", "device",
                 "No media in device", False, "insert_media"),
    ErrorPattern(r"[Rr]ead error", "read_error", "device",
                 "Device read error", False, "check_disk", "critical"),
    ErrorPattern(r"[Ww]rite error", "write_error", "device",
                 "Device write error", False, "check_disk", "critical"),
]

# Category 11: Compilation & Build Errors
BUILD_ERRORS = [
    ErrorPattern(r"[Nn]o rule to make target", "no_make_rule", "build",
                 "Make target not found", False, "check_makefile"),
    ErrorPattern(r"[Mm]issing separator", "missing_separator", "build",
                 "Makefile syntax error", False, "fix_makefile"),
    ErrorPattern(r"[Uu]ndefined reference", "undefined_reference", "build",
                 "Undefined symbol", True, "add_library"),
    ErrorPattern(r"[Ss]ymbol lookup error", "symbol_lookup", "build",
                 "Symbol not found", True, "fix_ldpath"),
    ErrorPattern(r"[Ll]ibrary not found", "library_not_found", "build",
                 "Library not found", True, "install_lib"),
    ErrorPattern(r"[Hh]eader.*not found", "header_not_found", "build",
                 "Header file not found", True, "install_dev"),
    ErrorPattern(r"[Rr]elocation error", "relocation_error", "build",
                 "Relocation error", True, "fix_ldpath"),
    ErrorPattern(r"[Cc]ompilation terminated", "compilation_failed", "build",
                 "Compilation failed", False, "check_errors"),
]

# Category 12: Archive & Compression Errors
ARCHIVE_ERRORS = [
    ErrorPattern(r"[Uu]nexpected end of file", "unexpected_eof_archive", "archive",
                 "Archive truncated", False, "redownload"),
    ErrorPattern(r"[Cc]orrupt archive", "corrupt_archive", "archive",
                 "Archive corrupted", False, "redownload"),
    ErrorPattern(r"[Ii]nvalid tar magic", "invalid_tar", "archive",
                 "Invalid tar archive", False, "check_format"),
    ErrorPattern(r"[Cc]hecksum error", "checksum_error", "archive",
                 "Checksum mismatch", False, "redownload"),
    ErrorPattern(r"[Nn]ot in gzip format", "not_gzip", "archive",
                 "Not gzip format", False, "check_format"),
    ErrorPattern(r"[Dd]ecompression failed", "decompress_failed", "archive",
                 "Decompression failed", False, "check_format"),
]

# Category 13: Shell Script Errors
SCRIPT_ERRORS = [
    ErrorPattern(r"[Bb]ad interpreter", "bad_interpreter", "script",
                 "Interpreter not found", True, "fix_shebang"),
    ErrorPattern(r"[Ll]ine \d+:.*command not found", "script_cmd_not_found", "script",
                 "Command in script not found", True, "install_dependency"),
    ErrorPattern(r"[Ii]nteger expression expected", "integer_expected", "script",
                 "Expected integer", False, "fix_syntax"),
    ErrorPattern(r"[Cc]onditional binary operator expected", "conditional_expected", "script",
                 "Expected conditional", False, "fix_syntax"),
]

# Category 14: Environment & PATH Errors
ENVIRONMENT_ERRORS = [
    ErrorPattern(r"[Vv]ariable not set", "var_not_set", "environment",
                 "Environment variable not set", True, "set_variable"),
    ErrorPattern(r"[Pp][Aa][Tt][Hh] not set", "path_not_set", "environment",
                 "PATH not configured", True, "set_path"),
    ErrorPattern(r"[Ee]nvironment corrupt", "env_corrupt", "environment",
                 "Environment corrupted", True, "reset_env"),
    ErrorPattern(r"[Ll]ibrary path not found", "lib_path_missing", "environment",
                 "Library path missing", True, "set_ldpath"),
    ErrorPattern(r"LD_LIBRARY_PATH", "ld_path_issue", "environment",
                 "Library path issue", True, "set_ldpath"),
]
...
CONFIG_ERRORS = [
    # Nginx errors
    ErrorPattern(r"nginx:.*\[emerg\]", "nginx_config_error", "config",
                 "Nginx configuration error", True, "fix_nginx_config"),
    ErrorPattern(r"nginx.*syntax.*error", "nginx_syntax_error", "config",
                 "Nginx syntax error", True, "fix_nginx_config"),
    ErrorPattern(r"nginx.*unexpected", "nginx_unexpected", "config",
                 "Nginx unexpected token", True, "fix_nginx_config"),
    ErrorPattern(r"nginx.*unknown directive", "nginx_unknown_directive", "config",
                 "Nginx unknown directive", True, "fix_nginx_config"),
    ErrorPattern(r"nginx.*test failed", "nginx_test_failed", "config",
                 "Nginx config test failed", True, "fix_nginx_config"),
    ErrorPattern(r"nginx.*could not open", "nginx_file_error", "config",
                 "Nginx could not open file", True, "fix_nginx_permissions"),
    # Apache errors
    ErrorPattern(r"apache.*syntax error", "apache_syntax_error", "config",
                 "Apache syntax error", True, "fix_apache_config"),
    ErrorPattern(r"apache2?ctl.*configtest", "apache_config_error", "config",
                 "Apache config test failed", True, "fix_apache_config"),
    ErrorPattern(r"[Ss]yntax error on line \d+", "config_line_error", "config",
                 "Config syntax error at line", True, "fix_config_line"),
    # MySQL/MariaDB errors
    ErrorPattern(r"mysql.*error.*config", "mysql_config_error", "config",
                 "MySQL configuration error", True, "fix_mysql_config"),
    # PostgreSQL errors
    ErrorPattern(r"postgres.*error.*config", "postgres_config_error", "config",
                 "PostgreSQL configuration error", True, "fix_postgres_config"),
    # Generic config errors
    ErrorPattern(r"configuration.*syntax", "generic_config_syntax", "config",
                 "Configuration syntax error", True, "fix_config_syntax"),
    ErrorPattern(r"invalid.*configuration", "invalid_config", "config",
                 "Invalid configuration", True, "fix_config_syntax"),
    ErrorPattern(r"[Cc]onfig.*parse error", "config_parse_error", "config",
                 "Config parse error", True, "fix_config_syntax"),
]

SERVICE_ERRORS = [
    ErrorPattern(r"[Ss]ervice failed to start", "service_failed", "service",
                 "Service failed to start", True, "check_service_logs"),
    ErrorPattern(r"[Uu]nit.*failed", "unit_failed", "service",
                 "Systemd unit failed", True, "check_service_logs"),
    ErrorPattern(r"[Jj]ob for.*\.service failed", "job_failed", "service",
                 "Service job failed", True, "check_service_logs"),
    ErrorPattern(r"[Ff]ailed to start.*\.service", "start_failed", "service",
                 "Failed to start service", True, "check_service_logs"),
    ErrorPattern(r"[Dd]ependency failed", "dependency_failed", "service",
                 "Service dependency failed", True, "start_dependency"),
    ErrorPattern(r"[Ii]nactive.*dead", "service_inactive", "service",
                 "Service not running", True, "start_service"),
    ErrorPattern(r"[Mm]asked", "service_masked", "service",
                 "Service is masked", True, "unmask_service"),
    ErrorPattern(r"[Ee]nabled-runtime", "service_enabled_runtime", "service",
                 "Service enabled at runtime", False, "check_service"),
    ErrorPattern(r"[Cc]ontrol process exited with error", "control_process_error", "service",
                 "Service control process failed", True, "check_service_logs"),
    ErrorPattern(r"[Aa]ctivation.*timed out", "activation_timeout", "service",
                 "Service activation timed out", True, "check_service_logs"),
]
...
UBUNTU_PACKAGE_MAP = {
    ...
    "nginx": "nginx", "apache2": "apache2", "httpd": "apache2",
    "mysql": "mysql-server", "mysqld": "mysql-server",
    "postgres": "postgresql", "psql": "postgresql-client",
    "redis": "redis-server", "redis-server": "redis-server",
    "mongo": "mongodb", "mongod": "mongodb",
    "node": "nodejs", "npm": "npm", "yarn": "yarnpkg",
    "python": "python3", "python3": "python3", "pip": "python3-pip", "pip3": "python3-pip",
    "docker": "docker.io", "docker-compose": "docker-compose",
    "git": "git", "curl": "curl", "wget": "wget",
    "vim": "vim", "nano": "nano", "emacs": "emacs",
    "gcc": "gcc", "g++": "g++", "make": "make", "cmake": "cmake",
    "java": "default-jdk", "javac": "default-jdk",
    "ruby": "ruby", "gem": "ruby",
    "go": "golang-go", "cargo": "cargo", "rustc": "rustc",
    ...
    "dig": "dnsutils", "nslookup": "dnsutils",
    "zip": "zip", "unzip": "unzip",
    "tar": "tar", "gzip": "gzip",
    "rsync": "rsync", "ssh": "openssh-client", "sshd": "openssh-server",
...
class ErrorDiagnoser:
    ...
    def extract_path_from_error(self, stderr: str, cmd: str) -> str | None:
        ...
        for part in cmd.split():
            if part.startswith("/") and any(c in part for c in ["/etc/", "/var/", "/usr/", "/home/", "/opt/", "/tmp/"]):
                return part
        ...
        for i, part in enumerate(cmd_parts):
            if part in ["systemctl", "service"]:
                for j in range(i + 1, len(cmd_parts)):
                    ...
                    if candidate not in ["start", "stop", "restart", "reload", "status", 
    ...
    def _generate_fix_commands(self, diagnosis: dict, cmd: str, stderr: str) -> None:
        ...
        # Docker strategies
            ...
            if container_name:
                ...
                diagnosis["suggestion"] = f"Container '{container_name}' already exists. Removing it and retrying."
            else:
                diagnosis["fix_commands"] = [
                    "docker ps -a",
            ...
            diagnosis["fix_commands"] = [
                f"docker volume create {volume_name}" if volume_name else "docker volume create <name>"
            ]
        
        elif strategy == "create_network":
            network_name = extracted.get("network")
            diagnosis["fix_commands"] = [
                f"docker network create {network_name}" if network_name else "docker network create <name>"
            ]
        ...
        # Timeout strategies
            ...
            interactive_patterns = ["docker exec -it", "docker run -it", "-ti ", "ollama run", "ollama chat"]
            ...
            if is_interactive:
                ...
                diagnosis["suggestion"] = "This command needs interactive input. Please run it in a separate terminal."
            else:
                diagnosis["fix_commands"] = [
                ...
                diagnosis["suggestion"] = "The operation timed out. This often happens with large downloads. You can retry manually."
...
class LoginHandler:
    ...
    def _save_credentials(self, service: str, credentials: dict[str, str]) -> None:
        ...
        for field, value in credentials.items():
            if value:  # Only save non-empty values
                encoded[field] = self._encode_credential(value)
    ...
    def _ask_use_saved(self, service: str, requirement: LoginRequirement) -> bool:
        ...
        for field in requirement.required_fields:
            if field in saved and saved[field]:
                if requirement.field_secret.get(field, False):
                    saved_fields.append(f"{field}=****")
                else:
                    value = saved[field]
    ...
    def _ask_save_credentials(self, service: str, credentials: dict[str, str]) -> None:
        ...
        console.print()
        console.print(f"[cyan]💾 Save these credentials for next time?[/cyan]")
        console.print(f"[dim]   Credentials will be stored in {self.CREDENTIALS_FILE}[/dim]")
        console.print(f"[dim]   (encoded, readable only by you)[/dim]")
    ...
    def check_env_credentials(self, requirement: LoginRequirement) -> dict[str, str]:
        ...
        found = {}
        for field, env_var in requirement.env_vars.items():
    ...
    def prompt_for_credentials(
        ...
        pre_filled: dict[str, str] | None = None
    ) -> dict[str, str] | None:
        """Prompt user for required credentials."""
        import getpass
        
        console.print()
        console.print(f"[bold cyan]🔐 {requirement.display_name} Authentication Required[/bold cyan]")
        console.print()
        ...
        if env_creds:
            console.print(f"[green]Found credentials in environment: {', '.join(env_creds.keys())}[/green]")
        ...
        try:
            for field in requirement.required_fields:
                if field in credentials and credentials[field]:
                    console.print(f"[dim]{requirement.field_prompts[field]}: (using existing)[/dim]")
                    continue
    ...
    def execute_login(
        ...
        if requirement.service in ["docker", "ghcr"]:
            ...
            console.print(f"[dim]Executing: docker login {registry or 'docker.io'} -u {username}[/dim]")
        ...
        console.print(f"[dim]Executing login...[/dim]")
        try:
    ...
    def handle_login(self, cmd: str, stderr: str) -> tuple[bool, str]:
        ...
        if self._has_saved_credentials(requirement.service):
            if self._ask_use_saved(requirement.service, requirement):
                ...
                console.print(f"[cyan]Using saved credentials...[/cyan]")
                success, stdout, login_stderr = self.execute_login(requirement, credentials)
                
                if success:
                    console.print(f"[green]✓ Successfully logged in to {requirement.display_name} using saved credentials[/green]")
                    return True, f"Logged in to {requirement.display_name} using saved credentials"
                else:
                    console.print(f"[yellow]Saved credentials didn't work: {login_stderr[:100] if login_stderr else 'Unknown error'}[/yellow]")
                    console.print(f"[dim]Let's enter new credentials...[/dim]")
                    credentials = None
                    used_saved = False
        ...
        if not credentials:
            ...
            if requirement.service in self.cached_credentials:
                ...
                for field in requirement.required_fields:
                    if field in saved and saved[field] and not requirement.field_secret.get(field, False):
                        pre_filled[field] = saved[field]
            
            credentials = self.prompt_for_credentials(requirement, pre_filled if pre_filled else None)
...
{{"fix_commands": ["docker rm -f ollama", "docker run ..."], "reasoning": "Container 'ollama' already exists, removing it first"}}"""
            ...
            if response and response.get("response_type") != "error":
                ...
                if response.get("answer"):
                    ...
                    for line in answer.split('\n'):
                        line = line.strip()
                        if line.startswith('$') or line.startswith('sudo ') or line.startswith('docker '):
    ...
    def auto_fix_error(
        ...
        while attempt < max_attempts and skipped_attempts < max_skips:
            ...
            if self._is_strategy_attempted(cmd, strategy, error_type):
                console.print(f"[dim]   Skipping already-tried strategy: {strategy} for {error_type}[/dim]")
                skipped_attempts += 1
            ...
            if fixed:
                ...
                if message == "Will retry with sudo":
                    ...
                    if self._is_fix_attempted(cmd, sudo_cmd):
                        console.print(f"[dim]   Already tried sudo, skipping...[/dim]")
                        skipped_attempts += 1
                        continue
                ...
                if not cmd.strip().startswith("sudo") and not self._is_fix_attempted(cmd, sudo_fallback):
                    ...
                    if success:
                        return True, f"Fixed with sudo fallback", all_commands_executed
                    
                    current_stderr = new_stderr
                    current_diagnosis = self.diagnoser.diagnose_error(cmd, new_stderr)
                else:
                    if cmd.strip().startswith("sudo"):
                        console.print(f"[dim]   Already running with sudo, no more fallbacks[/dim]")
                    else:
                        console.print(f"[dim]   Sudo fallback already tried[/dim]")
                    break
        ...
        return False, f"Could not fix after {attempt} attempts ({skipped_attempts} skipped as duplicates)", all_commands_executed
    ...
    def apply_single_fix(
        ...
        # === Use Sudo ===
        if strategy == "use_sudo" or error_type in ["permission_denied", "operation_not_permitted", "access_denied"]:
            if not cmd.strip().startswith("sudo"):
        ...
        if strategy == "install_package" or error_type == "command_not_found":
            missing_cmd = extracted.get("missing_command") or self.diagnoser.extract_command_from_error(stderr)
            if not missing_cmd:
        ...
        # === Clear Package Lock ===
        if strategy == "clear_lock" or error_type in ["dpkg_lock", "apt_lock", "could_not_get_lock"]:
        ...
        # === Start Service ===
        if strategy in ["start_service", "check_service"] or error_type in ["service_inactive", "service_not_running"]:
            service = extracted.get("service")
        ...
        # === Free Memory ===
        if strategy == "free_memory" or error_type in ["oom", "cannot_allocate", "memory_exhausted"]:
        ...
        config_error_types = [
            "config_syntax_error", "nginx_config_error", "nginx_syntax_error", 
            "nginx_unexpected", "nginx_unknown_directive", "nginx_test_failed",
            "apache_syntax_error", "apache_config_error", "config_line_error",
            "mysql_config_error", "postgres_config_error", "generic_config_syntax",
            "invalid_config", "config_parse_error", "syntax_error"
        ]
                ...
                if fixed:
                    ...
                    if "nginx" in error_type or "nginx" in cmd.lower():
                        ...
                        if v_success:
                            ...
                            console.print(f"[yellow]   Config still has errors[/yellow]")
        ...
        # === Network Fixes ===
        if category == "network":
            if strategy == "check_dns" or error_type in ["dns_temp_fail", "dns_unknown", "dns_failed"]:
            ...
            if strategy == "find_port_user" or error_type == "address_in_use":
                ...
                if port:
                    ...
                    return False, f"Port {port} is in use - kill the process first", commands_executed
        ...
        # === Wait and Retry ===
        if strategy == "wait_retry" or error_type in ["resource_unavailable", "text_file_busy", "device_busy"]:
            import time
            ...
            if llm_fix:
                ...
                if fix_commands:
                    ...
                    for fix_cmd in fix_commands:
                        ...
                        success, stdout, stderr = self._execute_command(fix_cmd, needs_sudo=needs_sudo)
                        commands_executed.append(fix_cmd)
                    ...
                    if commands_executed:
                        return True, f"Executed AI-suggested fixes", commands_executed
