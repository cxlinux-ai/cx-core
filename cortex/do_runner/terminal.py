...
class TerminalMonitor:
    ...
    def __init__(self, notification_callback: Callable[[str, str], None] | None = None, use_llm: bool = True):
        ...
        from cortex.do_runner.diagnosis import ErrorDiagnoser, AutoFixer
    ...
    def start(self, verbose: bool = True, show_live: bool = True, expected_commands: list[str] | None = None):
        """Start monitoring terminal for commands."""
        self.start_monitoring(expected_commands=expected_commands, verbose=verbose, show_live=show_live)
    ...
    def start_monitoring(self, expected_commands: list[str] | None = None, verbose: bool = True, show_live: bool = True, clear_old_logs: bool = True):
        ...
        if verbose:
            ...
            if service_running:
                setup_info = "[bold green]✓ Cortex Watch Service is running[/bold green]\n" \
                ...
                try:
                    ...
                    for clip_cmd in [["xclip", "-selection", "clipboard"], ["xsel", "--clipboard", "--input"]]:
                        try:
                            proc = subprocess.run(clip_cmd, input=short_cmd.encode(), capture_output=True, timeout=2)
                ...
                if hook_installed:
                    clipboard_msg = "[green]📋 Copied to clipboard![/green] " if clipboard_copied else ""
                    setup_info = "[green]✓ Terminal watch hook is installed in .bashrc[/green]\n" \
                    ...
                    setup_info = f"[bold yellow]⚠ For real-time monitoring in OTHER terminals:[/bold yellow]\n\n" \
                                f"[bold cyan]{short_cmd}[/bold cyan]\n\n" \
    ...
    def test_monitoring(self):
        ...
        console.print(f"  Tmux: {'[green]✓ available[/green]' if self._tmux_available else '[dim]not available[/dim]'}")
        console.print(f"  Screen: {'[green]✓ available[/green]' if self._screen_available else '[dim]not available[/dim]'}")
...
'''
        ...
        try:
            ...
            return True, f"✓ Installed system-wide to {profile_script}\n" \
...
''')
                ...
                if marker in content:
                    ...
                    for line in lines:
                        if marker in line:
                            ...
                            continue
                        if skip_until_blank:
                            if line.strip() == "" or line.startswith("export PROMPT") or line.startswith("cortex_watch") or line.startswith("precmd_functions"):
                                continue
                            if line.startswith("}"):
                                continue
                            skip_until_blank = False
    ...
    def _monitor_loop(self):
        ...
        while self._monitoring:
            ...
            if check_count % 150 == 0 and self._show_live_output:
                console.print(f"[dim]... still monitoring ({len(self._commands_observed)} commands observed so far)[/dim]")
        ...
        if output and len(output) > 5:
            ...
            for pattern, msg in error_patterns:
                ...
                if "✓" in output or "success" in output.lower() or "complete" in output.lower():
                    ...
                    if output_lines:
                        console.print(f"[dim]   Output: {output_lines[0][:60]}{'...' if len(output_lines[0]) > 60 else ''}[/dim]")
    ...
    def _provide_error_help(self, command: str, output: str):
        ...
        if not claude_analysis:
            ...
            if can_auto_fix:
                diag_table.add_row("Auto-Fix", f"[green]● Yes[/green] [dim]({fix_strategy})[/dim]" if fix_strategy else "[green]● Yes[/green]")
            else:
        ...
        if error_type == "unknown" and not claude_analysis and self._llm and self._use_llm and self._llm.is_available():
            ...
            if llm_help:
                ...
                if llm_fix:
                    console.print()
                    console.print(f"[bold green]💡 AI Suggested Fix:[/bold green] [cyan]{llm_fix}[/cyan]")
    ...
    def _run_auto_fix_commands(self, commands: list[str], original_command: str, error_type: str) -> bool:
        ...
        for i, fix_cmd in enumerate(actionable, 1):
            ...
            if needs_sudo:
                try:
                    ...
                    if check_sudo.returncode != 0:
                        ...
                        console.print(f"  [dim][{i}/{len(actionable)}][/dim] [yellow]![/yellow] {fix_cmd[:55]}... [dim](needs sudo)[/dim]")
                        continue
                except Exception:
                    sudo_commands_pending.append(fix_cmd)
                    results.append((fix_cmd, "sudo", None))
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    console.print(f"  [dim][{i}/{len(actionable)}][/dim] [green]✓[/green] {cmd_display}")
                else:
                    if "password" in (result.stderr or "").lower() or "terminal is required" in (result.stderr or "").lower():
                        sudo_commands_pending.append(fix_cmd)
                        results.append((fix_cmd, "sudo", None))
                        console.print(f"  [dim][{i}/{len(actionable)}][/dim] [yellow]![/yellow] {cmd_display} [dim](needs sudo)[/dim]")
                    else:
                        results.append((fix_cmd, "failed", result.stderr[:60] if result.stderr else "failed"))
                        all_success = False
                        console.print(f"  [dim][{i}/{len(actionable)}][/dim] [red]✗[/red] {cmd_display}")
                        console.print(f"      [dim red]{result.stderr[:80] if result.stderr else 'Command failed'}[/dim red]")
                        break
                    
            except subprocess.TimeoutExpired:
                results.append((fix_cmd, "timeout", None))
                all_success = False
                console.print(f"  [dim][{i}/{len(actionable)}][/dim] [yellow]⏱[/yellow] {cmd_display} [dim](timeout)[/dim]")
                break
            except Exception as e:
    ...
    def _is_safe_fix_command(self, command: str) -> bool:
        ...
        if cmd_lower.startswith("sudo "):
            ...
            safe_sudo = ["systemctl", "service", "apt", "apt-get", "nginx", "chmod", "chown", "mkdir"]
    ...
    def _send_error_notification(self, command: str, solution: str, error_type: str = "", can_auto_fix: bool = False):
...
Next command:"""
    ...
    def print_collected_context(self):
        ...
        with self._lock:
            ...
            for i, obs in enumerate(self._commands_observed[-10:], 1):  # Show last 10
                timestamp = obs.get("timestamp", "")[:19].split("T")[-1] if "T" in obs.get("timestamp", "") else obs.get("timestamp", "")[:8]
                source = obs.get("source", "unknown")
                command = obs.get("command", "")
                ...
                console.print(f"  [dim]{timestamp}[/dim] [cyan]{source:12}[/cyan] [white]{command[:50]}{'...' if len(command) > 50 else ''}[/white]")
            
            if len(self._commands_observed) > 10:
                console.print(f"  [dim]... and {len(self._commands_observed) - 10} more commands[/dim]")
            
            # Add AI analysis if available
            if self._llm and self._use_llm and self._llm.is_available() and len(self._session_context) >= 2:
                console.print()
    ...
    def _process_observed_command(self, command: str, source: str = "unknown"):
        ...
        with self._lock:
            recent = [c for c in self._commands_observed 
                     if c["command"] == command 
                     and (datetime.datetime.now() - datetime.datetime.fromisoformat(c["timestamp"])).seconds < 5]
            if recent:
                return
        ...
        if issues:
            ...
            if self.notification_callback:
                self.notification_callback(f"Cortex: Issue detected", issues)
    ...
    def _notify_wrong_command(self, wrong_command: str):
        ...
        if correct_cmd:
            ...
            console.print(f"         [bold yellow]📢 Expected command:[/bold yellow] [cyan]{correct_cmd}[/cyan]")
    ...
    def _check_command_result(self, command: str):
        ...
        if "systemctl" in cmd_lower:
            ...
            if "run" in cmd_lower or "start" in cmd_lower:
                ...
                if container_name:
                    check_cmd = f"docker ps -f name={container_name} --format '{{{{.Status}}}}' 2>&1"
                else:
        ...
        if check_cmd:
            try:
                ...
                error_indicators = [
                    "failed", "error", "not found", "inactive", "dead",
                    "could not", "unable", "denied", "cannot", "exited",
        ...
        if error_output:
            console.print(f"         [dim]checking...[/dim]")
