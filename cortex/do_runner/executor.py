...
class TaskTreeExecutor:
        ...
        for root_task in self.tree.root_tasks:
            ...
            if success:
                ...
                if root_task.failure_reason:
                    repair_details.append(f"- {root_task.command[:40]}...: {root_task.failure_reason}")
    ...
    def _execute_task_with_repair(
        ...
        if diagnosis.get("can_auto_fix") and task.repair_attempts < task.max_repair_attempts:
            ...
            if fix_commands:
                console.print(f"[cyan]🔧 Attempting auto-repair ({task.repair_attempts}/{task.max_repair_attempts})...[/cyan]")
                ...
                if new_paths and confirm_callback:
                    ...
                    if confirm_callback(repair_tasks):
                        ...
                        if all_repairs_success:
                            console.print(f"[cyan]↻ Retrying original command...[/cyan]")
                            success, output, error, duration = self._execute_command(task.command)
                            task.output = output
                            task.error = error
                            task.duration_seconds += duration
                            ...
                            if success:
                                task.status = CommandStatus.SUCCESS
                                task.reasoning = f"Auto-repaired after {task.repair_attempts} attempt(s)"
        ...
        if diagnosis.get("manual_suggestion") and notify_callback:
            ...
            if Confirm.ask("Would you like to run this manually while Cortex monitors?", default=False):
    ...
    def _needs_sudo(self, command: str) -> bool:
        ...
        sudo_keywords = [
            ...
            "chmod", "chown", "mount", "umount", "fdisk",
        ...
        for part in cmd_parts:
            for path in system_paths:
                if path in part:
                    if any(op in command for op in [">", ">>", "cp ", "mv ", "rm ", "mkdir ", "touch ", "sed ", "tee "]):
                        return True
    ...
    def _diagnose_error(
        ...
        if "permission denied" in error_lower:
            ...
            return {
                ...
                "can_auto_fix": True,
                "fix_commands": [f"sudo {command}"] if not command.strip().startswith("sudo") else [],
        ...
        if "command not found" in error_lower or "not found" in error_lower:
            ...
            return {
                ...
                "manual_suggestion": f"Install: sudo apt install {missing_cmd}" if missing_cmd else "Install the required command",
        ...
        return {
            ...
            "reasoning": f"The command failed with an unexpected error.",
        }
