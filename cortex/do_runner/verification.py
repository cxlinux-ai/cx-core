...
class ConflictDetector:
    ...
    def _check_docker_conflict(self, cmd: str, purpose: str) -> dict[str, Any]:
            ...
            if name_match:
                ...
                if success and container_id.strip():
                    ...
                    _, image_info, _ = self._execute_command(
                        f"docker inspect --format '{{{{.Config.Image}}}}' {container_name}", needs_sudo=False
                    )
        ...
        if "docker-compose" in cmd.lower() or "docker compose" in cmd.lower():
            if "up" in cmd:
                success, services, _ = self._execute_command("docker compose ps -q 2>/dev/null", needs_sudo=False)
                if success and services.strip():
                    return self._create_conflict_result(
                        ...
                        is_active=True,
                        alternative_actions=[
                            {"action": "use_existing", "description": "Keep existing services", "commands": []},
                            {"action": "restart", "description": "Restart services", "commands": ["docker compose restart"]},
                            {"action": "recreate", "description": "Recreate services", "commands": ["docker compose down", "docker compose up -d"]},
                        ]
                    )
    ...
    def _check_file_conflict(self, cmd: str, purpose: str) -> dict[str, Any]:
        ...
        for path in paths_in_cmd:
            ...
            is_write_cmd = any(p in cmd for p in [">" , "tee ", "cp ", "mv ", "touch ", "mkdir ", "echo "])
            ...
            if is_write_cmd and os.path.exists(path):
                ...
                return self._create_conflict_result(
                    ...
                    is_active=True,
                    alternative_actions=[
                        {"action": "use_existing", "description": f"Keep existing {resource_type}", "commands": []},
                        {"action": "backup", "description": f"Backup and overwrite", "commands": [f"sudo cp -r {path} {path}.cortex.bak"]},
                        {"action": "recreate", "description": f"Remove and recreate", "commands": [f"sudo rm -rf {path}" if is_dir else f"sudo rm -f {path}"]},
                    ]
                )
    ...
    def _check_package_conflict(self, cmd: str, purpose: str) -> dict[str, Any]:
        ...
        if "apt install" in cmd or "apt-get install" in cmd:
            ...
            if pkg_match:
                ...
                success, _, _ = self._execute_command(f"dpkg -l {package} 2>/dev/null | grep -q '^ii'", needs_sudo=False)
                if success:
                    # Get version
                    _, version_out, _ = self._execute_command(f"dpkg -l {package} | grep '^ii' | awk '{{print $3}}'", needs_sudo=False)
                    ...
                    return self._create_conflict_result(
                        ...
                        is_active=True,
                        alternative_actions=[
                            {"action": "use_existing", "description": f"Keep current version ({version})", "commands": []},
                            {"action": "upgrade", "description": "Upgrade to latest version", "commands": [f"sudo apt install --only-upgrade -y {package}"]},
                            {"action": "reinstall", "description": "Reinstall package", "commands": [f"sudo apt install --reinstall -y {package}"]},
                        ]
                    )
        ...
        if "pip install" in cmd or "pip3 install" in cmd:
            ...
            if pkg_match:
                package = pkg_match.group(1)
                success, version_out, _ = self._execute_command(f"pip3 show {package} 2>/dev/null | grep Version", needs_sudo=False)
                if success and version_out:
                    ...
                    return self._create_conflict_result(
                        ...
                        alternative_actions=[
                            {"action": "use_existing", "description": f"Keep current version ({version})", "commands": []},
                            {"action": "upgrade", "description": "Upgrade to latest", "commands": [f"pip3 install --upgrade {package}"]},
                            {"action": "reinstall", "description": "Reinstall package", "commands": [f"pip3 install --force-reinstall {package}"]},
                        ]
                    )
        ...
        if "npm install -g" in cmd or "npm i -g" in cmd:
            ...
            if pkg_match:
                package = pkg_match.group(1)
                success, version_out, _ = self._execute_command(f"npm list -g {package} 2>/dev/null | grep {package}", needs_sudo=False)
                if success and version_out:
                    return self._create_conflict_result(
                        ...
                        alternative_actions=[
                            ...
                            {"action": "upgrade", "description": "Update to latest", "commands": [f"npm update -g {package}"]},
                        ]
                    )
    ...
    def _check_port_conflict(self, cmd: str, purpose: str) -> dict[str, Any]:
        ...
        for pattern in port_patterns:
            ...
            if match:
                ...
                success, output, _ = self._execute_command(f"ss -tlnp | grep ':{port} '", needs_sudo=True)
                if success and output:
                    ...
                    return self._create_conflict_result(
                        ...
                        is_active=True,
                        alternative_actions=[
                            {"action": "use_different", "description": f"Use a different port", "commands": []},
                            {"action": "stop_existing", "description": f"Stop process using port {port}", "commands": [f"sudo fuser -k {port}/tcp"]},
                        ]
                    )
    ...
    def _check_user_conflict(self, cmd: str, purpose: str) -> dict[str, Any]:
        ...
        if "useradd" in cmd or "adduser" in cmd:
            ...
            if user_match:
                ...
                success, _, _ = self._execute_command(f"id {username} 2>/dev/null", needs_sudo=False)
                if success:
                    return self._create_conflict_result(
                        resource_type="user",
                        ...
                        is_active=True,
                        alternative_actions=[
                            {"action": "use_existing", "description": f"Use existing user '{username}'", "commands": []},
                            {"action": "modify", "description": f"Modify existing user", "commands": []},
                        ]
                    )
        ...
        if "groupadd" in cmd or "addgroup" in cmd:
            ...
            if group_match:
                ...
                success, _, _ = self._execute_command(f"getent group {groupname} 2>/dev/null", needs_sudo=False)
                if success:
                    return self._create_conflict_result(
                        resource_type="group",
                        ...
                        is_active=True,
                        alternative_actions=[
                            {"action": "use_existing", "description": f"Use existing group '{groupname}'", "commands": []},
                        ]
                    )
            ...
            if venv_match:
                ...
                if os.path.exists(venv_path) and os.path.exists(os.path.join(venv_path, "bin", "python")):
                    return self._create_conflict_result(
                        ...
                        is_active=True,
                        alternative_actions=[
                            {"action": "use_existing", "description": f"Use existing venv", "commands": []},
                            {"action": "recreate", "description": "Delete and recreate", "commands": [f"rm -rf {venv_path}"]},
                        ]
                    )
    ...
    def _check_database_conflict(self, cmd: str, purpose: str) -> dict[str, Any]:
        ...
        if "mysql" in cmd.lower() and "create database" in cmd.lower():
            db_match = re.search(r'create\s+database\s+(?:if\s+not\s+exists\s+)?(\S+)', cmd, re.IGNORECASE)
            if db_match:
                ...
                if success and dbname in output:
                    return self._create_conflict_result(
                        ...
                        is_active=True,
                        alternative_actions=[
                            {"action": "use_existing", "description": f"Use existing database", "commands": []},
                            {"action": "recreate", "description": "Drop and recreate", "commands": [f"mysql -e 'DROP DATABASE {dbname}'"]},
                        ]
                    )
            ...
            if db_match:
                ...
                if success:
                    return self._create_conflict_result(
                        ...
                        alternative_actions=[
                            {"action": "use_existing", "description": f"Use existing database", "commands": []},
                            {"action": "recreate", "description": "Drop and recreate", "commands": [f"dropdb {dbname}"]},
                        ]
                    )
        ...
        if "crontab" in cmd or "/etc/cron" in cmd:
            ...
            if "echo" in cmd and ">>" in cmd:
                ...
                if job_match:
                    ...
                    success, crontab, _ = self._execute_command("crontab -l 2>/dev/null", needs_sudo=False)
                    if success and crontab:
                        ...
                        if job_cmd and job_cmd in crontab:
                            return self._create_conflict_result(
                                ...
                                is_active=True,
                                alternative_actions=[
                                    {"action": "use_existing", "description": "Keep existing cron job", "commands": []},
                                    {"action": "replace", "description": "Replace existing job", "commands": []},
                                ]
                            )
...
class VerificationRunner:
    ...
    def run_verification_tests(
        ...
        if "apache" in configs_to_check:
            ...
            success, stdout, stderr = self._execute_command("apache2ctl configtest", needs_sudo=True)
            test_results.append({
        ...
        if any(x in query_lower for x in ["proxy", "forward", "port", "listen"]):
            ...
            if port_match:
                ...
                if stdout.strip() not in ["failed", "000", ""]:
                    console.print(f"[green]   ✓ Port {port} responding (HTTP {stdout.strip()})[/green]")
                    test_results.append({
                    ...
                    console.print(f"[yellow]   ⚠ Port {port} not responding (may be expected)[/yellow]")
...
class FileUsefulnessAnalyzer:
    ...
    def check_file_exists_and_usefulness(
        ...
        for file_path in target_files:
            ...
            if os.path.exists(file_path):
                ...
                success, content, _ = self._execute_command(f"cat '{file_path}' 2>/dev/null", needs_sudo=True)
                ...
                if success and content:
                    ...
                    if useful_parts["is_useful"]:
                        ...
                        console.print(f"[cyan]   ✓ Contains useful content: {useful_parts['summary']}[/cyan]")
                ...
                if parent_dir and not os.path.exists(parent_dir):
                    console.print(f"[yellow]📁 Parent directory doesn't exist: {parent_dir}[/yellow]")
                    result["recommendations"].append({
                        "file": file_path,
    ...
    def analyze_file_usefulness(
        ...
        # Nginx configuration
        if any(x in content_lower for x in ["server {", "location", "nginx", "proxy_pass", "listen"]):
        ...
        elif any(x in content_lower for x in ["<virtualhost", "documentroot", "apache", "servername"]):
            result["is_useful"] = True
