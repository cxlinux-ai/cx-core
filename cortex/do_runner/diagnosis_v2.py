...
class DiagnosisEngine:
    ...
    def _is_url_based_permission_error(self, command: str, stderr: str) -> tuple[bool, str | None, str | None]:
        """
        ...
        is_remote_command = any(
            ...
            for pattern in self.URL_COMMAND_PATTERNS
        )
        ...
        is_url_based = bool(is_remote_command and found_host) or any([
            "401" in stderr,
            "403" in stderr,
        ...
        ]) or any(git_remote_patterns) or any(aws_patterns)
        
        if is_url_based:
            console.print(f"[cyan]   🌐 Detected URL-based permission error[/cyan]")
    ...
    def _is_local_file_permission_error(self, command: str, stderr: str) -> tuple[bool, str | None]:
        ...
        for pattern in local_patterns:
            ...
            if match:
                ...
                if path.startswith("/") or path.startswith("./"):
                    console.print(f"[cyan]   📁 Detected local file permission error[/cyan]")
        ...
        if path_match and "permission denied" in stderr.lower():
            ...
            console.print(f"[cyan]   📁 Detected local file permission error (from command)[/cyan]")
            console.print(f"[dim]      Path: {path}[/dim]")
            return True, path
    ...
    def _resolve_permission_error_type(
        ...
        if is_url:
            ...
            elif "aws " in command.lower() or "az " in command.lower() or "gcloud " in command.lower():
...
EXAMPLE OUTPUT:
{
    ...
    def _generate_fallback_fix_plan(
        ...
        if category == ErrorCategory.PERMISSION_DENIED_LOCAL:
            path = info.get("path", "")
            reasoning = f"Local file/directory permission denied - using elevated privileges"
            commands.append(FixCommand(
        ...
        # Legacy generic permission denied - try to determine type
            ...
            commands.append(FixCommand(
                ...
                purpose=f"Install package providing the command",
                requires_sudo=True,
            ))
    ...
    def _try_system_info(self, var_name: str, command: str, diagnosis: DiagnosisResult) -> str | None:
...
Respond with ONLY the value, nothing else. If you cannot determine the value, respond with "UNKNOWN"."""
        ...
        try:
            ...
            if login_cmd:
                ...
                if result.returncode == 0:
                    ...
                    if self._login_handler and Confirm.ask("Save credentials for future use?", default=True):
                        self._login_handler._save_credentials(host, {
                            "username": username,
                            "password": password,
                        })
    ...
    def execute_fix_commands(
        ...
        for i, fix_cmd in enumerate(fix_plan.commands, 1):
            ...
            if unresolved:
                console.print(f"[yellow]   ⚠ Skipping command with unresolved variables: {unresolved}[/yellow]")
            ...
            try:
                ...
                console.print(f"[red]   ✗ Timeout after 120s[/red]")
                results.append(ExecutionResult(
                    command=command,
                    success=False,
                    stdout="",
    ...
    def diagnose_and_fix(
        ...
        while self.error_stack:
            ...
            if entry.fix_attempts >= self.MAX_FIX_ATTEMPTS:
                console.print(f"[red]✗ Max fix attempts ({self.MAX_FIX_ATTEMPTS}) reached for command[/red]")
                self.pop_error()
                continue
            ...
            if diagnosis.category in url_auth_categories:
                console.print(f"[cyan]🌐 URL-based access error detected - handling authentication[/cyan]")
                ...
                if auth_success:
                    ...
                    if test_result.returncode == 0:
                        ...
                        console.print(f"[yellow]⚠ New error after login, continuing diagnosis...[/yellow]")
                        continue
                else:
            ...
            if test_result.returncode == 0:
                console.print(f"[green]✓ Original command now succeeds![/green]")
                self.pop_error()
                ...
                console.print(f"[yellow]⚠ Original command still fails[/yellow]")
                
                if new_error != entry.error:
                    console.print(f"[cyan]   New error detected, updating...[/cyan]")
    ...
    def _print_diagnosis(self, diagnosis: DiagnosisResult, command: str) -> None:
        ...
        table.add_row("Error", diagnosis.error_message[:100] + "..." if len(diagnosis.error_message) > 100 else diagnosis.error_message)
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        ("systemctl start myservice", "Failed to start myservice.service: Unit myservice.service not found."),
    ]
