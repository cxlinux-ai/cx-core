...
class FallbackProgress:
    ...
    def __init__(self):
        """
        Initialize the fallback progress indicator's internal state used for a text-based spinner.
        
        Sets up the spinner character sequence and index, the current message, a running flag, the background thread reference, and a Lock to protect shared state during concurrent updates.
        """
    ...
    def start(self, message: str):
        """
        Begin displaying a progress spinner with the given message.
        
        Starts a background thread that updates the progress display until stopped; calling this sets the current message and marks the spinner as running.
        
        Parameters:
            message (str): Text to show alongside the spinner.
        """
        with self._lock:
    ...
    def _animate(self):
        """
        Continuously renders a spinner and the current message to stdout until the spinner is stopped.
        
        This method loops, updating the displayed spinner frame and message at regular intervals (approximately every 0.1s), and exits when the internal running flag is cleared. It performs output using stdout and flushes after each update.
        """
        while True:
            with self._lock:
    ...
    def stop(self, final_message: str = ""):
        """
        Stop the progress indicator and print a completion line.
        
        Parameters:
            final_message (str): Optional message to display after stopping; if empty, the last shown spinner message is used.
        """
        with self._lock:
            self._running = False
            thread = self._thread
    ...
    def fail(self, message: str = ""):
        """
        Stop the spinner and print a failure line for the current operation.
        
        If `message` is provided it will be used; otherwise the last displayed message is used. This also stops the background spinner thread.
        Parameters:
            message (str): Optional failure message to display; uses the current message when empty.
        """
        with self._lock:
            self._running = False
            thread = self._thread
...
def get_progress_indicator() -> ProgressIndicator:
    """
    Get the module-level ProgressIndicator singleton, creating it on first access in a thread-safe manner.
    
    Returns:
        ProgressIndicator: The global ProgressIndicator instance.
    """
