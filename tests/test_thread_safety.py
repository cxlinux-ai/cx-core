...
def test_singleton_thread_safety_transaction_history():
    ...
    def get_instance():
        """
        Obtain the transaction history singleton and record its identity.
        
        Appends the singleton object's id() to the enclosing `results` list for later uniqueness checks.
        """
        history = get_history()
...
def test_singleton_thread_safety_hardware_detection():
    ...
    def get_instance():
        """
        Record the identity of the hardware detector by appending its object id to the surrounding `results` list.
        
        This helper obtains the global detector and appends `id(detector)` to the `results` list captured from the enclosing scope.
        """
...
def test_singleton_thread_safety_degradation_manager():
    ...
    def get_instance():
        """
        Record the identity of the global degradation manager instance.
        
        Appends the object's id returned by get_degradation_manager() to the surrounding `results` list so concurrent callers can verify singleton identity.
        """
...
def test_connection_pool_concurrent_reads():
    """
    Verify that the SQLite connection pool returns consistent results under concurrent read load.
    
    Sets up a temporary SQLite database with 100 rows, creates a connection pool (pool_size=5), and launches 20 threads that each perform 50 SELECT COUNT(*) reads. Asserts every read returns 100 and ensures the pool and temporary database file are cleaned up.
    """
    ...
    try:
        ...
        def read_data(thread_id: int):
            """
            Read the row count from the `test` table 50 times using a connection from the pool.
            
            Parameters:
                thread_id (int): Caller thread identifier used to correlate results with the caller.
            
            Returns:
                list[int]: A list of 50 integers, each the result of `SELECT COUNT(*) FROM test` observed on each read.
            """
            results = []
            for _ in range(50):
                with pool.get_connection() as conn:
...
def test_connection_pool_concurrent_writes():
    ...
    try:
        ...
        def write_data(thread_id: int):
            """
            Insert 20 rows into the test table using the shared connection pool, recording any errors.
            
            Parameters:
                thread_id (int): Identifier used to set the `thread_id` column and to distinguish inserted values.
            
            Details:
                For i from 0 to 19, obtains a connection from the surrounding `pool`, inserts a row with `thread_id`
                and `value` = "thread_{thread_id}_value_{i}", and commits each insert. On exception, appends a tuple
                (thread_id, error_message) to the surrounding `errors` list.
            """
            try:
                for i in range(20):
                    with pool.get_connection() as conn:
...
def test_hardware_detection_parallel():
    ...
    def detect_hardware():
        """
        Detect hardware using the shared detector and record the CPU core count or any error.
        
        Appends the detected CPU core count (uses 1 if detection reports 0) to the outer-scope list `results`. If an exception occurs, appends the exception message string to the outer-scope list `errors`.
        """
        try:
            detector = get_detector()
...
def test_connection_pool_timeout():
    """
    Verify that the SQLite connection pool raises a TimeoutError when all connections are in use.
    
    Creates a temporary database and a pool configured with pool_size=2 and timeout=0.5, holds both connections via the pool's context-manager API, and asserts that attempting to acquire a third connection raises a TimeoutError with the message "Could not acquire database connection". Cleans up held connections, closes the pool, and removes the temporary database file.
    """
    from cortex.utils.db_pool import get_connection_pool
...
def test_stress_concurrent_operations():
    ...
    try:
        ...
        def mixed_operations(thread_id: int):
            """
            Perform 50 database operations mixing reads and writes against the shared `pool`.
            
            Each iteration performs approximately 70% SELECT COUNT(*) reads and 30% INSERT writes that record the calling thread's id and a timestamp. On error, appends a tuple (thread_id, error_message) to the shared `errors` list.
            
            Parameters:
                thread_id (int): Identifier used in inserted row data and included in any error records.
            """
            try:
                for i in range(50):
